...
const STYLE_CONFIG: Record<string, StyleConfig> = {
  ...
  'Vintage Poster': {
    ...
    tintStyle: { background: 'rgba(80,45,5,0.18)', mixBlendMode: 'multiply' as React.CSSProperties['mixBlendMode'] },
    titleStyle: { color: '#f5e6c8', fontFamily: '"Playfair Display", serif', fontWeight: 400, letterSpacing: '0.09em', textShadow: '0 2px 10px rgba(0,0,0,0.85)' },
    authorStyle: { color: 'rgba(220,195,150,0.85)', fontFamily: '"EB Garamond", serif', textShadow: '0 1px 6px rgba(0,0,0,0.75)' },
    titleYPct: 16,
    authorYPct: 92,
  },
}
...
export default function CoverGallery({ variants, title, author, onSelect, onBack, onLoadMore, onRegenerateAll, isLoadingMore, nextBatch }: Props) {
  ...
  return (
    <div className="flex-1 flex flex-col bg-zinc-900 overflow-auto">
      ...
      {/* Generate more / Regenerate all button */}
      <div className="flex justify-center px-8 pb-8 pt-2 shrink-0">
        {nextBatch >= 3 ? (
          <button
            onClick={onRegenerateAll}
            disabled={isLoadingMore || loadingCount > 0}
            className="flex items-center gap-2 px-5 py-2.5 rounded-full border border-zinc-600 text-sm text-zinc-300 hover:border-indigo-500 hover:text-indigo-300 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
          >
          ...
          >
            {isLoadingMore ? (
              ...
              <>
                ...
                {nextBatch === 2 ? 'Try wildcard styles' : 'Generate more styles'}
              </>
            )}
          </button>
        )}
